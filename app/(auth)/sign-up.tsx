...
export default function SignUp() {
  ...
  const handleSignUp = async () => {
    try {
      ...
      setError(Object.fromEntries(Object.entries(validateForm).map(([key, value]) => [key, value.message])))
    ...
    >
      <VStack space="xl" className="w-full max-w-md">
        ...
        <FormControl className="w-full mb-1" isInvalid={!!error.name}>
          ...
          {!!error.name && <Text className="text-red-500 text-sm mt-1">{error.name}</Text>}
        </FormControl>
        ...
        <FormControl className="w-full mb-1" isInvalid={!!error.email}>
          <Text className="mb-2 text-sm font-medium text-gray-700">Email (không bắt buộc)</Text>
          <Input variant="outline" size="md">
            <InputField
          ...
          {!!error.email && <Text className="text-red-500 text-sm mt-1">{error.email}</Text>}
        </FormControl>
        ...
        <FormControl className="w-full mb-6" isInvalid={!!error.role}>
          ...
          <Select selectedValue={user.role} onValueChange={(value) => handleChange("role", value)}>
            <SelectTrigger>
          ...
          {!!error.role && <Text className="text-red-500 text-sm mt-1">{error.role}</Text>}
        </FormControl>
