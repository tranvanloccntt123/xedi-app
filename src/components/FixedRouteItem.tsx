    ...
    <VStack space="xs" className={`mx-2 bg-white p-4 rounded-md ${className}`}>
      ...
      <VStack>
        <HStack className="items-center">
          ...
          <Text className="font-black font-normal text-sm">{item.startLocation.display_name}</Text>
        </HStack>
        ...
        <HStack className="items-center">
          ...
          <Text className="font-black font-normal text-sm">{item.endLocation.display_name}</Text>
        </HStack>
      </VStack>
      <Text className="text-sm text-gray-600">Giá: {formatMoney(item.price)} VND</Text>
      {!disabled && (
        <HStack space="sm" className="justify-end mt-2">
          <Button size="xs" className="bg-blue-500 rounded-lg" onPress={() => router.push(`/fixed/${item.id}/detail`)}>
