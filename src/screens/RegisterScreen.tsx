...
export default function RegisterScreen() {
  ...
  return (
    <Box className="flex-1 bg-white p-6">
      ...
      <Text className="text-sm text-gray-600 mb-6">Đăng kí ngay đặt xe liền tay</Text>
      <VStack space="md" className="mt-5 bg-white p-8 rounded-lg shadow-md items-center justify-center">
        <Text className="text-3xl font-bold mb-6 text-blue-600">Register</Text>
        <Text className="text-sm text-gray-600 mb-4">Registering as: {role}</Text>
        ...
        <Button onPress={handleRegister} className="w-full bg-blue-500 rounded-md" style={AppStyles.btn}>
          <Text className="text-white font-semibold">Register</Text>
        </Button>
        {error ? <Text className="text-red-500 mt-2">{error}</Text> : null}
        <Button variant="outline" onPress={() => navigation.navigate(RouteName.Login)} className="w-full mt-4 border-blue-500 rounded-md" style={AppStyles.btn}>
