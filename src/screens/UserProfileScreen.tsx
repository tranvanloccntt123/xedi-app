...
export function UserProfileScreen() {
  ...
  useEffect(() => {
    const fetchUser = () => {
      const fetchedUser = realm.objectForPrimaryKey<User>(
        "User",
          ...
          <Card className="mb-6 p-4">
            ...
            <VStack space="md">
              ...
              <Text className="text-lg">
                <Text className="font-semibold">Total Ratings:</Text>{" "}
                {user.totalRatings || 0}
              </Text>
              <Text className="text-lg">
                <Text className="font-semibold">Average Rating:</Text>{" "}
                {user.averageRating?.toFixed(2) || "N/A"}
              </Text>
            </VStack>
          </Card>
